...
var /** holds major version number for IE or NaN for real browsers */
    ...
    /** @name angular */
    angular           = window.angular || (window.angular = {}),//定义angular，window.angular={} 是一个赋值语句，
    angularModule,
    nodeName_,
    uid               = 0;
...
function angularInit(element, bootstrap) {
  ...
  if (appElement) {
    ...
    bootstrap(appElement, module ? [module] : [], config);//调用bootstrap方法
  }
}
...
function bootstrap(element, modules, config) {
  ...
  var doBootstrap = function() {
    ...
    modules.unshift('ng');
    var injector = createInjector(modules, config.strictDi);//创建注射器
    injector.invoke(['$rootScope', '$rootElement', '$compile', '$injector',//调用
       function(scope, element, compile, injector) {
        scope.$apply(function() {
          element.data('$injector', injector);
          compile(element)(scope);//编译指令
        });
      }]
    );
    return injector;
...
function bindJQuery() {
  ...
  jQuery = window.jQuery;//首先确认有没有jquery
  // Use jQuery if it exists with proper functionality, otherwise default to us.
  // Angular 1.2+ requires jQuery 1.7.1+ for on()/off() support.
  if (jQuery && jQuery.fn.on) {
    jqLite = jQuery;//如果已经导入了jquery，就是使用外部使用的jquery
    extend(jQuery.fn, {
...
function setupModuleLoader(window) {
  ...
  return ensure(angular, 'module', function() {
     ...
     */
    return function module(name, requires, configFn) {//在angular.module时真实调用的方法
...
function publishExternalAPI(angular){
  extend(angular, {//这里的angular是一个空对象，extend是拷贝的意思，拷贝下面的工具函数
    'bootstrap': bootstrap,
    'copy': copy,
    'extend': extend,
    'equals': equals,
  ...
  angularModule = setupModuleLoader(window);//给angular构建了模块加载器
  try {
...
function createInjector(modulesToLoad, strictDi) {//创建注射器
  ...
  function provider(name, provider_) {//provider是基础
  ...
  function factory(name, factoryFn) { return provider(name, { $get: factoryFn }); }//调用provider
  ...
  function decorator(serviceName, decorFn) {//装饰过程
  ...
  function createInternalInjector(cache, factory) {//创建injecter
...
var styleDirective = valueFn({
  ...
  if (window.angular.bootstrap) {//判断angularjs是否已经启动，如果启动就报下面的error
  ...
  bindJQuery();//调用jquery方法

  publishExternalAPI(angular);//将angular公共扩展的api加载到angular全局函数上

  jqLite(document).ready(function() {//启动angular，默认从document开始启动
